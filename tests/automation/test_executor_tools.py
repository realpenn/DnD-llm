...
def test_cleric_blessed_strikes_divine_strike_adds_selected_damage_once_per_turn(
    make_state,
) -> None:
    state = make_state()
    assert state.encounter is not None
    ...
    character.actions.extend(["srd.blessed_strikes", "srd.blessed_strikes_divine_strike"])
    target = state.encounter.combatants["goblin1"]
    target.hp_current = 30
    target.hp_max = 30
    target.armor_class = 1
    ...
    result = tools.perform_action(
        ...
        params={
            ...
            "divine_strike_damage_type": "necrotic",
        },
        idempotency_key="cleric-blessed-strikes-divine-strike",
    )
    ...
    marker = next(
        change
        for change in result["state_changes"]
        if change["type"] == "blessed_strikes_divine_strike_used"
    )
    assert marker["damage_type"] == "necrotic"
    assert any(
        effect["condition"] == "blessed_strikes_divine_strike_used"
        for effect in state.encounter.combatants["pc1"].status_effects
    )

    with pytest.raises(AutomationError, match="can be used only once per turn"):
        tools.perform_action(
            "pc1",
            "srd.shortsword_attack",
            ["goblin1"],
            ...
            idempotency_key="cleric-blessed-strikes-divine-strike-repeat",
        )


def test_cleric_blessed_strikes_divine_strike_validates_request(
    make_state,
) -> None:
    state = make_state()
    assert state.encounter is not None
    ...
    with pytest.raises(AutomationError, match="divine_strike_damage_type"):
        tools.perform_action(
            "pc1",
            "srd.shortsword_attack",
            ["goblin1"],
            params={"use_blessed_strikes_divine_strike": True},
            idempotency_key="cleric-blessed-strikes-missing-damage-type",
        )

    with pytest.raises(AutomationError, match="requires a weapon attack"):
        tools.perform_action(
            "pc1",
            "srd.sacred_flame",
            ["goblin1"],
            ...
            idempotency_key="cleric-blessed-strikes-non-weapon",
        )

    character.feature_choices = {"cleric.blessed_strikes": "potent_spellcasting"}
    with pytest.raises(AutomationError, match="Divine Strike option"):
        tools.perform_action(
            "pc1",
            "srd.shortsword_attack",
            ["goblin1"],
            ...
            idempotency_key="cleric-blessed-strikes-wrong-choice",
        )
...
def test_cleric_blessed_strikes_potent_spellcasting_adds_wisdom_to_cleric_cantrip(
    ...
    character.abilities["wis"] = 18
    character.feature_choices = {"cleric.blessed_strikes": "potent_spellcasting"}
    character.actions.extend(["srd.sacred_flame", "srd.blessed_strikes_potent_spellcasting"])
    target = state.encounter.combatants["goblin1"]
    target.hp_current = 30
    target.hp_max = 30
    ...
    result = tools.perform_action(
        ...
        "srd.sacred_flame",
        ["goblin1"],
        idempotency_key="cleric-blessed-strikes-potent-spellcasting",
    )
    ...
    assert damage_change["passive_sources"] == [
        {
            "source_action_id": "srd.blessed_strikes_potent_spellcasting",
            "modifier": "cleric_potent_spellcasting",
            "spell_id": "srd.spell.sacred_flame",
            "amount": 4,
        }
    ]
...
def test_cleric_blessed_strikes_potent_spellcasting_does_not_boost_non_cleric_cantrip(
    ...
    character.abilities["wis"] = 18
    character.feature_choices = {"cleric.blessed_strikes": "potent_spellcasting"}
    character.actions.extend(["srd.fire_bolt", "srd.blessed_strikes_potent_spellcasting"])
    target = state.encounter.combatants["goblin1"]
    target.hp_current = 30
    target.hp_max = 30
    target.armor_class = 1
    ...
    result = tools.perform_action(
        ...
        "srd.fire_bolt",
        ["goblin1"],
        idempotency_key="cleric-blessed-strikes-non-cleric-cantrip",
    )

    damage_change = next(change for change in result["state_changes"] if change["type"] == "damage")
    assert damage_change["amount"] == 4
    assert "passive_damage_bonus" not in damage_change
    assert target.hp_current == 26
...
def test_cloak_of_protection_item_requirement_accepts_equipment_and_rejects_missing(
    ...
    with pytest.raises(AutomationError, match="actor does not have item srd.cloak_of_protection"):
        tools.use_item(
            "pc1",
            "srd.cloak_of_protection",
            ["pc1"],
            action_id="srd.wear_cloak_of_protection",
            idempotency_key="missing-cloak-of-protection",
        )
    ...
    result = tools.use_item(
        ...
        ["pc1"],
        action_id="srd.wear_cloak_of_protection",
        idempotency_key="equipped-cloak-of-protection",
    )
...
def test_cloak_of_protection_adds_ac_and_saving_throw_bonus(make_state) -> None:
    state = make_state()
    assert state.encounter is not None
    character = state.characters["pc1"]
    character.inventory["srd.cloak_of_protection"] = 1
    character.armor_class = 14
    state.encounter.combatants["pc1"].armor_class = 14
    compendium = CompendiumLoader("rules_data").load()
    tools = EngineTools(state, compendium, AuditLog())
    ...
    result = tools.use_item(
        ...
        ["pc1"],
        action_id="srd.wear_cloak_of_protection",
        idempotency_key="wear-cloak-of-protection",
    )
    attack = AutomationExecutor(
        state,
    ...
    save_action = ActionDefinition(
        id="test.cloak_of_protection_save",
        name="Cloak of Protection Save",
        localization={"en": "Cloak of Protection Save", "zh": "防护斗篷豁免", "aliases": []},
        source="test",
        rules_version="test",
        action_type="test",
        action_economy="none",
    ...
    direct_save = tools.roll_save(
        ...
        "dex",
        difficulty_tier="medium",
        idempotency_key="cloak-of-protection-direct-save",
    )

    assert result["success"] is True
    assert character.inventory["srd.cloak_of_protection"] == 1
    effect = state.encounter.combatants["pc1"].status_effects[-1]
    assert effect["source_action_id"] == "srd.wear_cloak_of_protection"
    assert effect["duration"] == {"until": "while_wearing_cloak_of_protection"}
    assert effect["passive_modifiers"] == {
        "armor_class_bonus": 1,
        "saving_throw_bonus": 1,
    }
    ...
    assert save_node["passive_adjustment"] == 1
    assert save_node["passive_sources"][0]["modifier"] == "saving_throw_bonus"
    assert save_node["passive_sources"][0]["source_action_id"] == ("srd.wear_cloak_of_protection")
    assert direct_save["passive_bonus"] == 1
    assert direct_save["passive_bonus_sources"][0]["source_action_id"] == (
        "srd.wear_cloak_of_protection"
    )
...
def test_arcane_eye_records_concentration_visual_sensor_world_effect(make_state) -> None:
    ...
    result = tools.cast_spell(
        ...
        [],
        4,
        idempotency_key="cast-arcane-eye",
    )
    ...
    assert cost_change["resource"] == "spell_slot_4"
    effect = state.world.active_effects[-1]
    assert effect["source_action_id"] == "srd.arcane_eye"
    assert effect["effect_type"] == "arcane_eye_sensor"
    assert effect["concentration"] is True
    assert effect["scope"] == {"target": "point_within_range", "range_ft": 30}
    assert effect["duration"] == {"until": "concentration_1_hour"}
    assert effect["tick_on"] == "self_turn_end"
    assert effect["metadata"] == {
        "invisible": True,
    ...
    assert world_effect_change["effect_type"] == "arcane_eye_sensor"
    assert world_effect_change["concentration"] is True
