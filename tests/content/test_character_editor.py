...
def test_natural_language_character_edit_assigns_fighter_two_extra_attacks_at_level_11() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 战士11")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"fighter": 11}
    assert "srd.extra_attack" in result.character.actions
    assert "srd.two_extra_attacks" in result.character.actions
    assert result.character.resources["srd.resource.indomitable"] == 1


def test_natural_language_character_edit_assigns_fighter_studied_attacks_at_level_13() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 战士13")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"fighter": 13}
    assert "srd.two_extra_attacks" in result.character.actions
    assert "srd.studied_attacks" in result.character.actions
...
def test_natural_language_character_edit_assigns_fighter_three_extra_attacks_at_level_20() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 战士20")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"fighter": 20}
    assert "srd.two_extra_attacks" in result.character.actions
    assert "srd.three_extra_attacks" in result.character.actions
    assert result.character.resources["srd.resource.action_surge"] == 2
    assert result.character.resources["srd.resource.indomitable"] == 3
...
def test_natural_language_character_edit_assigns_cleric_improved_blessed_strikes() -> None:
    character = default_fighter("pc1", "Penn")
...
def test_natural_language_character_edit_assigns_barbarian_brutal_strike() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 barbarian9")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"barbarian": 9}
...
def test_natural_language_character_edit_assigns_barbarian_improved_brutal_strike() -> None:
    ...
    result = apply_natural_language_character_edit(character, "职业 barbarian13")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"barbarian": 13}
    assert "srd.brutal_strike" in result.character.actions
...
def test_natural_language_character_edit_assigns_barbarian_indomitable_might() -> None:
    ...
    result = apply_natural_language_character_edit(character, "职业 barbarian18")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"barbarian": 18}
    assert "srd.persistent_rage" in result.character.actions
...
def test_natural_language_character_edit_assigns_monk_self_restoration() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 monk10")
...
def test_natural_language_character_edit_assigns_monk_disciplined_survivor() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 monk14")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"monk": 14}
    assert "srd.deflect_energy" in result.character.actions
    assert "srd.disciplined_survivor" in result.character.actions
    assert result.character.resources["srd.resource.focus_points"] == 14
    assert monk_disciplined_survivor_applies(result.character) is True


def test_natural_language_character_edit_assigns_monk_perfect_focus() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 monk15")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"monk": 15}
    assert "srd.disciplined_survivor" in result.character.actions
...
def test_natural_language_character_edit_assigns_hunter_superior_hunters_prey_at_level_11() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 ranger11 子职 hunter")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"ranger": 11}
    assert result.character.subclasses == {"ranger": "hunter"}
...
def test_natural_language_character_edit_assigns_ranger_feral_senses_at_level_18() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 ranger18")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"ranger": 18}
    assert "srd.feral_senses" in result.character.actions
    assert "srd.precise_hunter" in result.character.actions
    assert ranger_feral_senses_blindsight_ft(result.character) == 30


def test_natural_language_character_edit_does_not_grant_feral_senses_too_early() -> None:
    character = default_fighter("pc1", "Penn")
...
def test_natural_language_character_edit_does_not_grant_hunter_superior_prey_too_early() -> None:
    ...
    result = apply_natural_language_character_edit(character, "职业 ranger10 子职 hunter")

    assert result.accepted is True
    assert result.character is not None
    assert "srd.superior_hunters_prey" not in result.character.actions


def test_natural_language_character_edit_does_not_grant_hunter_superior_prey_without_hunter() -> (
    None
):
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 ranger11")
...
def test_natural_language_character_edit_assigns_thief_supreme_sneak_at_level_nine() -> None:
    ...
    assert "srd.thiefs_reflexes" not in result.character.actions


def test_natural_language_character_edit_assigns_thief_reflexes_at_level_seventeen() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 rogue17 子职 thief")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"rogue": 17}
    assert result.character.subclasses == {"rogue": "thief"}
    assert "srd.supreme_sneak" in result.character.actions
    assert "srd.thiefs_reflexes" in result.character.actions
...
def test_natural_language_character_edit_assigns_champion_superior_critical_at_level_15() -> None:
    ...
    result = apply_natural_language_character_edit(character, "职业 战士15 子职 champion")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"fighter": 15}
    assert result.character.subclasses == {"fighter": "champion"}
    assert "srd.improved_critical" in result.character.actions
    assert "srd.superior_critical" in result.character.actions


def test_natural_language_character_edit_assigns_champion_heroic_warrior_at_level_10() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 战士10 子职 champion")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"fighter": 10}
    assert result.character.subclasses == {"fighter": "champion"}
    assert "srd.heroic_warrior" in result.character.actions
    assert "srd.superior_critical" not in result.character.actions
...
def test_natural_language_character_edit_assigns_rogue_slippery_mind() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 rogue15")
...
def test_natural_language_character_edit_assigns_rogue_stroke_of_luck() -> None:
    character = default_fighter("pc1", "Penn")

    result = apply_natural_language_character_edit(character, "职业 rogue20")

    assert result.accepted is True
    assert result.character is not None
    assert result.character.class_levels == {"rogue": 20}
    assert "srd.stroke_of_luck" in result.character.actions
    assert result.character.resources["srd.resource.stroke_of_luck"] == 1
    assert rogue_stroke_of_luck_applies(result.character) is True
