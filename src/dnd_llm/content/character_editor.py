...
DEFENSIVE_TACTICS_RE = re.compile(
    r"(?:defensive\s+tactics|防御战术)\s*[:=：]?\s*(escape\s+the\s+horde|multiattack\s+defense|[A-Za-z_-]+|[\u4e00-\u9fff]+)",
    re.IGNORECASE,
)
...
BLESSED_STRIKES_RE = re.compile(
    r"(?:blessed\s+strikes|祝福打击|受祝打击)\s*[:=：]?\s*"
    r"(divine\s+strike|potent\s+spellcasting|[A-Za-z_-]+|[\u4e00-\u9fff]+)",
    re.IGNORECASE,
)
...
def apply_natural_language_character_edit(
    ...
    for defensive_tactics_match in DEFENSIVE_TACTICS_RE.finditer(text):
        result = set_hunter_defensive_tactics_choice(
            edited,
            defensive_tactics_match.group(1).strip(),
        )
        errors.extend(result.errors)
    ...
    for blessed_strikes_match in BLESSED_STRIKES_RE.finditer(text):
        result = set_cleric_blessed_strikes_choice(
            edited,
            blessed_strikes_match.group(1).strip(),
        )
        errors.extend(result.errors)
    ...
    for weapon_mastery_match in WEAPON_MASTERY_RE.finditer(text):
        choices = _weapon_mastery_choices_from_segment(
            _segment_after_match(text, weapon_mastery_match)
        )
        result = set_weapon_mastery_choices(edited, choices)
        errors.extend(result.errors)
...
def _weapon_mastery_choices_from_segment(segment: str) -> list[str]:
    return [
        choice.strip()
        for choice in re.split(r"\s*(?:,|，|、|/|\band\b|和)\s*", segment, flags=re.IGNORECASE)
        if choice.strip()
    ]
